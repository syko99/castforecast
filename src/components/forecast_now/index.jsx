...
const ForecastNow = ({ forecast, updateForecast, locationName }) => {
    ...
    let wind = Math.round(currentForecast.wind)
    ...
    return (
        ...
        dark:md:bg-white/10'>
            ...
            <div className='w-32 mt-8'>
                <WeatherIcon hour={currentHour} precip={currentForecast.precip} cloudCover={currentForecast.cloudCover}/>
            </div>
            <p className='text-7xl relative md:text-8xl'>{temp}<span className="absolute">&deg;</span></p>
            <p className='mb-4'>{cloudCover}</p>
            <p>{city}, {territory}</p>
            <p className='lowercase'>{wind}mph winds</p>
            <p>
